...
export default function Input({ label, hint, error, ...props }: Props) {
  return (
    <label className="grid gap-1">
      ...
      <input
        {...props}
        className={`bg-[#131313] border ${error ? 'border-red-500' : 'border-border'} text-text px-3 py-2 rounded-xl font-inherit focus:outline-none focus:ring-2 focus:ring-gold/60`}
      />
